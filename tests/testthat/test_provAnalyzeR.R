...
context("provAnalyzeR")

# === HELPER FUNCTIONS ======================================================= #

# helper function to get expected output for backward lineages
# uses test case: preexisting
get.expected.backward <- function()
{
  # no backwards lineage for a and b (preexisting variales)
...
# no provenance
test_that("generate.preexisting.summary - no/empty provenance", 
          {
            ...
            expect_error(provAnalyzeR::generate.preexisting.summary(provParseR::get.preexisting(.analyze.env$prov))("x"))
            
            # empty provenance
            c0 <- system.file("testdata", "empty.json", package = "provAnalyzeR")
            expect_error(provAnalyzeR::prov.analyze.file(c0))
            expect_false(provAnalyzeR:::.analyze.env$has.graph)
            expect_error(provAnalyzeR::generate.preexisting.summary(provParseR::get.preexisting(.analyze.env$prov))("x"))
          })

# # no data nodes
# test_that("analyze.lineage - no data nodes",
#           {
#             skip("analyze.lineage - no data nodes")
#             
...
#             
#             c2 <- utils::capture.output(c1 <- analyze.variable(all = TRUE))
#             
#             expect_null(c1)
...
# 
# # analyze.lineage - all (backward)
# test_that("analyze.lineage - all (backward)",
#           {
#             # all
#             c2 <- utils::capture.output(
#               c1 <- analyze.lineage(all = TRUE))
#             
...
#               c3 <- analyze.lineage("a", "d", "invalid", all = TRUE))
#             
...
#             c5 <- analyze.lineage(start.line = 12, all = TRUE)[[1]]
...
#               c6 <- analyze.lineage(start.line = 100, all = TRUE))
#             
#             expect_null(c6)
...
#               c8 <- analyze.lineage(start.line = c(1,2), all = TRUE))
#             
#             e8 <- e.backward[c(1,2)]
#             
...
#               c10 <- analyze.lineage(script.num = 1, all = TRUE))
#             
...
#               c12 <- analyze.lineage(script.num = 100, all = TRUE))
#             
#             expect_null(c12)
...
#               c14 <- analyze.lineage(start.line = c(1,2), script.num = c(1,2), all = TRUE))
#             
...
#           })
# 
# # analyze.lineage - all (forward)
# test_that("analyze.lineage - all (forward)",
#           {
#             # all
#             c1 <- analyze.lineage(all = TRUE, forward = TRUE)
#             
...
#             
#             # with a name queries (no lineage, valid, invalid)
#             c2 <- analyze.lineage("output", "a", "invalid", all = TRUE, forward = TRUE)
#             
...
#             
#             # start line query (valid)
#             c3 <- analyze.lineage(start.line = 19, all = TRUE, forward = TRUE)[[1]]
...
#               c4 <- analyze.lineage(start.line = 100, all = TRUE, forward = TRUE))
#             
#             expect_null(c4)
...
#             
#             # start line query (multiple)
#             c6 <- analyze.lineage(start.line = c(18,19), all = TRUE, forward = TRUE)
#             
...
#             
#             # script num query (valid)
#             c7 <- analyze.lineage(script.num = 1, all = TRUE, forward = TRUE)
#             
...
#             # script num query (invalid)
#             c9 <- utils::capture.output(
#               c8 <- analyze.lineage(script.num = 100, all = TRUE, forward = TRUE))
#             
#             expect_null(c8)
#             expect_true(nchar(paste(c9, collapse='\n')) > 0)
#             
#             
#             # script num query (multiple)
#             c10 <- analyze.lineage(script.num = c(1:2), all = TRUE, forward = TRUE)
#             
...
#           })
# 
# # analyze.lineage - name queries (backward)
# test_that("analyze.lineage - name queries (backward)",
#           {
#             # valid
#             c1 <- analyze.lineage(f)[[1]]
#             e1 <- e.backward$f
#             
#             expect_equivalent(c1, e1)
#             
#             # invalid
#             c3 <- utils::capture.output(c2 <- analyze.lineage("invalid"))
#             
#             expect_null(c2)
#             expect_true(nchar(paste(c3, collapse='\n')) > 0)
#             
#             # no backward lineage
#             c5 <- utils::capture.output(c4 <- analyze.lineage("a"))
#             
#             expect_null(c4)
#             expect_true(nchar(paste(c5, collapse='\n')) > 0)
#             
#             # multiple queries
#             c7 <- utils::capture.output(
#               c6 <- analyze.lineage("a", f, "invalid.1", "e", "invalid.2"))
#             
#             e6 <- e.backward[c(5,4)]
#             
#             expect_equivalent(c6, e6)
...
#             c9 <- utils::capture.output(
#               c8 <- analyze.lineage("f", start.line = 10))
#             
#             expect_null(c8)
#             expect_true(nchar(paste(c9, collapse='\n')) > 0)
#           })
# 
# # analyze.lineage - name queries (forward)
# test_that("analyze.lineage - name queries (forward)",
#           {
#             # valid
#             c1 <- analyze.lineage(b, forward = TRUE)[[1]]
#             e1 <- e.forward$b
#             
#             expect_equivalent(c1, e1)
#             
...
#               c2 <- analyze.lineage("invalid", forward = TRUE))
#             
#             expect_null(c2)
#             expect_true(nchar(paste(c3, collapse='\n')) > 0)
#             
#             # multiple queries (valid, invalid)
#             c4 <- analyze.lineage("b", "invalid", output, forward = TRUE)
#             
#             e4 <- e.forward[c(13,4)]
#             
...
#               c5 <- analyze.lineage("b", start.line = 50))
#             
#             expect_null(c5)
#             expect_true(nchar(paste(c6, collapse='\n')) > 0)
#           })
# 
# # analyze.lineage - start line queries (backward)
# test_that("analyze.lineage - start line queries (backward)",
#           {
#             # valid
#             c1 <- analyze.lineage("f", start.line = 6)[[1]]
#             e1 <- e.backward$f
#             
#             expect_equivalent(c1, e1)
#             
#             # invalid
#             c3 <- utils::capture.output(
#               c2 <- analyze.lineage("f", start.line = 100))
#             
#             expect_null(c2)
#             expect_true(nchar(paste(c3, collapse='\n')) > 0)
#             
#             # multiple
#             c4 <- analyze.lineage("e", "f", start.line = c(5,6))
#             e4 <- e.backward[c(4,5)]
#             
#             expect_equivalent(c4, e4)
#             
#             # neither the first nor last instance of variable
#             c5 <- analyze.lineage("dev.2", start.line = 20)[[1]]
...
#           })
# 
# # analyze.lineage - start line queries (forward)
# test_that("analyze.lineage - start line queries (forward)",
#           {
#             # valid
#             c1 <- analyze.lineage("b", start.line = 25, forward = TRUE)[[1]]
#             e1 <- e.forward$b
#             
#             expect_equivalent(c1, e1)
#             
...
#             # invalid
#             c3 <- utils::capture.output(
#               c2 <- analyze.lineage("b", start.line = 50, forward = TRUE))
#             
#             expect_null(c2)
#             expect_true(nchar(paste(c3, collapse='\n')) > 0)
#             
#             
#             # multiple
#             c4 <- analyze.lineage("output", "g", start.line = c(3,25), forward = TRUE)
#             e4 <- e.forward[c(4,14)]
#             
#             expect_equivalent(c4, e4)
#             
#             
#             # neither the first nor last instance of variable
#             c5 <- analyze.lineage("dev.2", start.line = 12, forward = TRUE)[[1]]
#             c5 <- c5[ , -4]   # omit code column from comparison
#             
...
#           })
# 
# # analyze.lineage - script num queries (backward)
# test_that("analyze.lineage - script num queries (backward)",
#           {
#             # valid
#             c1 <- analyze.lineage("e", "f", script.num = 1)
#             e1 <- e.backward[c(4,5)]
#             
#             expect_equivalent(c1, e1)
#             
...
#             # invalid
#             c3 <- utils::capture.output(
#               c2 <- analyze.lineage("output", script.num = 2))
#             
#             expect_null(c2)
#             expect_true(nchar(paste(c3, collapse='\n')) > 0)
#             
#             
#             # multiple
#             c4 <- analyze.lineage("f", script.num = c(1,2))[[1]]
#             e4 <- e.backward$f
#             
#             expect_equivalent(c4, e4)
#           })
# 
# # analyze.lineage - script num queries (forward)
# test_that("analyze.lineage - script num queries (forward)",
#           {
#             # valid
#             c1 <- analyze.lineage(b, g, script.num = 1, forward = TRUE)
...
#             # invalid
#             c3 <- utils::capture.output(
#               c2 <- analyze.lineage("output", script.num = 2, forward = TRUE))
#             
#             expect_null(c2)
#             expect_true(nchar(paste(c3, collapse='\n')) > 0)
#             
#             
#             # multiple
#             c4 <- analyze.lineage(b, script.num = c(1,2), forward = TRUE)[[1]]
#             e4 <- e.forward$b
#             
#             expect_equivalent(c4, e4)
#           })
...
#           {	
#             # valid (includes a preexisting variable)
#             c1 <- provAnalyzeR:::.get.lineage("d3")
#             e1 <- c("p3", "p4")
#             
#             expect_equivalent(c1, e1)
#             
...
#             c2 <- provAnalyzeR:::.get.lineage("d2")
#             expect_null(c2)
#             
#             
#             # lineage with only 1 proc node
#             c3 <- provAnalyzeR:::.get.lineage("d20")
#             expect_equivalent(c3, "p20")
#             
#             
#             # non-assignment with lineage of 1 node
#             c4 <- provAnalyzeR:::.get.lineage("d22")
...
#           {
#             # valid
#             c1 <- provAnalyzeR:::.get.lineage("d15", forward = TRUE)
#             e1 <- c("p16", "p17", "p18", "p19")
#             
#             expect_equivalent(c1, e1)
#             
#             
#             # valid (a preexisting variable)
#             c2 <- provAnalyzeR:::.get.lineage("d19", forward = TRUE)
#             e2 <- c("p20", "p21")
#             
...
#             c3 <- provAnalyzeR:::.get.lineage("d4", forward = TRUE)
